...
export default function AttractionForm({ mode = 'create', attractionId, initialData }: AttractionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation
    if (!name.trim()) {
      setError('Nama destinasi tidak boleh kosong');
      return;
    }
    ...
    try {
      const url = mode === 'create' ? '/api/admin/attractions' : `/api/admin/attractions/${attractionId}`;
      ...
      if (!response.ok) {
        throw new Error(data.error || `Gagal ${mode === 'create' ? 'membuat' : 'memperbarui'} destinasi`);
      }

      setSuccess(true);

      // Redirect to attractions page after 2 seconds
      setTimeout(() => {
    ...
    return (
      <div className="rounded-lg bg-green-50 p-8 text-center text-green-900">
        <p className="mb-2 text-2xl font-bold">
          ✓ Destinasi berhasil {mode === 'create' ? 'ditambahkan' : 'diperbarui'}!
        </p>
        <p className="text-sm">Anda akan diarahkan kembali ke daftar destinasi...</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <Textarea
          placeholder="Jelaskan tentang destinasi ini, atraksi utama, aktivitas yang bisa dilakukan, jam operasional, dll..."
          value={description}
      ...
      <div>
        ...
        <Input
          type="text"
          placeholder="Masukkan lokasi/alamat destinasi..."
          value={location}
      ...
      <div>
        <ImageUpload
          ...
          hint="Drag and drop atau klik untuk memilih foto destinasi wisata"
          maxFiles={10}
        />
      </div>
